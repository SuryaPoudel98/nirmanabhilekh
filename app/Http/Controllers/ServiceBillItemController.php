...
class ServiceBillItemController extends Controller
{
    ...
    {
        $Expenses = ExpensesFromEquipment::select('amount')->where('cancel', 0)->get();
        $Income = IncomeFromEquipment::select('amount')->where('cancel', 0)->get();
        $Invoice = ProjectActivity::select('debit','credit')->where('cancel',0)->get();
        $serviceBillAmount = DB::table('service_billing_amounts')
            ->select('alltotalamount')
            ->where('service_billing_amounts.status', '=', 0)
            ->where('service_billing_amounts.cancel', '=', 0)
            ->get();
